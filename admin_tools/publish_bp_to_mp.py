...
def remove_platform_data(bp_spec):
    for substrate in bp_spec["resources"]["substrate_definition_list"]:
        ...
        if substrate['type'] == "AHV_VM":
            ...
            #    substrate["create_spec"]["resources"]["guest_customization"]["cloud_init"]["user_data"] = ""
            ...
            for image in substrate["create_spec"]["resources"]["disk_list"]:
                if "data_source_reference" in image and image["data_source_reference"]:
                    if image["data_source_reference"]["kind"] != "app_package":
        ...
        if substrate["type"] == "AZURE_VM":
            ...
            for disk in substrate["create_spec"]["resources"]["storage_profile"]["data_disk_list"]:
                ...
                disk["name"] = "@@{vmname}@@-data-disk-@@{calm_unique_hash}@@-" + str(random.random())[-3:]
...
def approve_marketplace_bp(marketplace_json, projects=[], category=None):
    ...
    marketplace_json["metadata"]["categories"] = {
            "AppFamily": category
        }
...
if __name__ == "__main__":
    ...
    publish_bp_as_existing_marketplace_bp(blueprint_json, marketplace_bp_name,
        ...
        auto_approve=auto_approve, projects=project.split(','), category=category, icon_name=icon)