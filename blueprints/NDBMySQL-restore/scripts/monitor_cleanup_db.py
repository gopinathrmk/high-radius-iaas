...
for x in range(20):
    ...
    if not resp.ok:
        print("Couldn't retrieve Detail of Cleanup DB Operation : '{}' !!!".format(cleanup_operation_id))
    ...
    if resp_content['percentageComplete'] == "100":
        print("Cleanup DB Operation Completed With Message:", resp_content['message'])
        break    

# If the operation did not complete within 10 minutes, assume it's not successful and error out
if resp_content['percentageComplete'] != "100":
    print("Waited for 10 mins and Cleanup DB is not completed. Exiting")
