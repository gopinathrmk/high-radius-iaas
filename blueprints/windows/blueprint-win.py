"""
Windows Server Build on AHV Multi-VM Blueprint
"""
...
BP_CRED_windows_default = basic_cred(blueprint_variables["windows_default_creds_username"], password=blueprint_variables['windows_default_creds_secret'], name="windows_default", default=True)
...
class AHVSubstrate_Default(Substrate):
    ...
    readiness_probe = readiness_probe(connection_type="POWERSHELL",disabled=False,retries="10",connection_port=5985,address="@@{platform.status.resources.nic_list[0].ip_endpoint_list[0].ip}@@",delay_secs="60",credential=ref(BP_CRED_windows_default),)
...
class Common(Profile):
    ...
    OS = CalmVariable.WithOptions(["WIN2022", "WIN2019"],label="Please select the OS version for the VM",default="WIN2022",is_mandatory=True,is_hidden=False,runtime=True,description="",)
...
Windows_BP.__doc__ = read_file('mp_meta/bp-description.md')

def main():
    print(Windows_BP.json_dumps(pprint=True))
